 ...
 *   판정을 코드처럼 지어내지 않는다 (RetestFailure.kt). **예외가 하나 있다: `'AD_DISMISSED'`**
 *   (KAN-196). 보상형 광고를 끝까지 보지 않고 닫은 경우인데, 서버에 닿기 전에 네이티브 안에서
 *   끝나는 실패라 봉투가 없고 네이티브가 이 코드를 직접 만든다. `retryable: true`,
 *   `retryAfterMs: null`로 오고 문구 정본은 네이티브다 ("광고를 끝까지 보시면 다시 테스트할 수
 *   있어요") — 웹은 여기서도 `message`를 그대로 그릴 뿐 코드로 문구를 고르지 않는다

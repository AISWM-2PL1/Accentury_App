 ...
 * 사용자는 고르기를 반복하는데 화면은 꿈쩍도 하지 않는다. 적어도 그 화면에서는 선택이
 * 지켜지는 편이 맞다.
 *
 * **사본이 사는 범위는 다음 화면 이동 전까지다** (PR #109 리뷰 (2026-09-13)에서 정정). 모듈
 * 변수라 문서가 리로드되면 사라지는데, 이 앱의 화면 전환이 곧 문서 리로드다 —
 * `App.tsx`의 `assignHref`가 `window.location.href`에 대입한다. 그래서 저장이 막힌 브라우저는
 * 인트로에서 「허용」을 골라도 대기 화면의 슬롯이 `unknown`을 읽고, 광고 요청 자체가 나가지
 * 않는다 (`adsense.ts`의 `pauseAdRequests=1`). 안전한 방향의 실패라 방침 문제는 없다 —
 * 묻지 않은 것처럼 다루는 것뿐이고, 시트도 방문마다 다시 뜬다. 저장 위치가 브라우저
 * 저장소라고 적었으니, 그 저장소가 없으면 기억도 없다.
 */
...
/**
 * 이 문서의 사본. null이면 아직 이 실행에서 고른 적이 없다는 뜻이고, 그때만 저장소를 본다.
 * 다음 화면으로 넘어가면 문서가 리로드되어 여기서부터 다시 시작한다 (헤더 주석 참고).
 */
...
export function writeWebAdConsent(
  ...
  try {
    ...
    // 저장소가 막힌 환경(사생활 모드·쿼터 초과). 이 화면까지는 사본으로 이어진다.
  }
  return true
}

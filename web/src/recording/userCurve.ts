 ...
 *
 * ## 앱과 다른 한 가지: 중심을 이 녹음에서 잡는다
 *
 * 앱은 시작 게이트의 목소리 점검 화면(`VoiceCheckController`)이 "안녕하세요" 한 마디에서 중심
 * 음높이를 확정해 모든 문항에 물려준다 (`pitch-curve.md` §5). 웹에는 아직 그 화면이 없어
 * [userCurveDisplayPoints]가 **이 녹음의 처음 8개 유성 프레임**으로 중심을 잡는다. 대가는
 * 같은 사람의 곡선이 문항마다 조금씩 다른 축에 놓일 수 있다는 것이다. 웹 목소리 점검이 생기면
 * `centerHz` 인자로 넘기면 되도록 시그니처를 앱과 같게 열어 두었다.
 */
 ...
 * 잡는다(웹은 목소리 점검이 없어 항상 후자다 — 파일 헤더). **둘 다 없으면 빈 결과다** —

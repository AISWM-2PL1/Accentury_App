 ...
 * ## 유니온에 없는 공통 파라미터 하나
 *
 * `test_id`는 여기 적지 않는다. 응시가 시작된 뒤의 모든 이벤트에 창구(`track.ts`)가 공통으로
 * 얹는 값이라, 지점마다 다시 적어야 하는 값이 아니다 — 필드로 두면 이벤트가 하나 늘 때 그것만
 * 빠뜨린 이벤트가 순서 분석에서 조용히 사라진다 (`testId.ts`, KAN-33 AC 1).
 *
...
export type AnalyticsEvent =
  ...
  /**
   * 결과 화면에서 [개발팀에 후기 보내기]를 열었다 (KAN-211).
   *
   * 아래 [feedback_submitted]와 짝이다 — 둘의 차이가 곧 「열어 보고 안 쓴 비율」이고, 그 값이
   * 후기 칸을 계속 둘지 판단할 근거다. 하나만 세면 응답 수는 알아도 그 수가 적은 이유가
   * 자리를 못 찾아서인지 쓰다 말아서인지 갈리지 않는다.
   *
   * 파라미터가 없다. 어느 결과였는지는 공통으로 얹히는 `test_id`가 말하고, 그 밖에 이 사건에
   * 붙일 익명 축이 없다.
   */
   ...
   * 후기를 실제로 보냈다 (KAN-211).
   *
   * **본문과 이메일은 절대 싣지 않는다.** 서술 후기는 사람이 자유롭게 적는 글이라 무엇이든
   * 들어올 수 있고 이메일은 그 자체로 개인을 특정한다 — 이 파일 머리의 익명 규칙에서 가장
   * 명백하게 걸리는 값들이다. 후기 내용을 읽는 곳은 슬랙 알림과 DB뿐이고(1·2단계), 계측은
   * 「보냈다」는 사실과 별점만 안다.
   *
   * 별점은 1~5의 다섯 값짜리 집계 축이라 개인을 특정하지 않는다. 고르지 않았으면 null이고,
   * 그 자리는 GA4에서 `(not set)`으로 세어진다 (`campaign`과 같은 규칙).
   */
  | { name: 'feedback_submitted'; rating: number | null }
  // ── 운영 지표 (재검토 트리거의 계기판) ──────────────────────────────────
  /**

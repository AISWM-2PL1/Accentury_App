 ...
 * 때문이다. 앱은 [com.accentury.app.audio.RecordingEngine.MAX_DURATION_MS]를, 웹은 문항
 * 정의의 `maxDurationMs`를 쥐고 있다 - 여기에 10초를 적어 두면 서버가 상한을 올리는 날 세
 * 클라이언트에 낡은 숫자가 하나씩 남는다.
 *
...
fun reviewWindow(
    ...
    if (first == null || last == null || last <= first) return ReviewWindow(frames, floorMs)

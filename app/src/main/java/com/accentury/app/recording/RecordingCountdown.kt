...
/**
 * 녹음 카운트다운 계산 (KAN-161 4단계). 화면이 "지금 몇 초인가"와 "경고를 띄울 때인가"를
 * 판정하는 데 쓰는 순수 함수 묶음이다.
 *
 * Composable 안에 두지 않은 이유는 경계 때문이다. 여기서 틀리면 화면에는 "8초에 경고가 안
 * 떴다"나 "3초 남았다고 나온다"로만 드러나는데, 그 증상을 만드는 것은 부등호 하나와 반올림
 * 방향 하나다 — 사람 눈으로는 매번 스톱워치를 들고 확인해야 하지만 함수로 빼면 경계 네 개를
 * 테스트가 고정한다 (`RecordingCountdownTest`).
 *
 * 값과 규칙은 웹 `web/src/progress/WebVoiceRecorder.tsx`와 같다. 한 테스트 안에서 네이티브
 * 녹음 화면과 웹 녹음 패널이 번갈아 나오므로, 경고가 시작하는 순간이 두 런타임에서 다르면
 * 사용자에게는 같은 문항이 다르게 동작하는 것으로 보인다.
 */
 ...
 */
const val WARN_REMAINING_MS = 2_000L

/** 상한까지 남은 시간 (ms). 상한을 넘긴 뒤에도 음수로 내려가지 않는다 */
internal fun remainingMs(elapsedMs: Long, maxDurationMs: Long): Long =
    (maxDurationMs - elapsedMs).coerceAtLeast(0L)
 ...
 */
internal fun isCountdownWarning(elapsedMs: Long, maxDurationMs: Long): Boolean =
    remainingMs(elapsedMs, maxDurationMs) <= WARN_REMAINING_MS
 ...
 * 머무는 시간은 한 프레임 남짓이다.
 */
internal fun remainingSeconds(elapsedMs: Long, maxDurationMs: Long): Int =
    ceil(remainingMs(elapsedMs, maxDurationMs) / 1000.0).toInt()

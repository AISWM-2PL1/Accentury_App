...
/**
 * 광고 로드의 세대 카운터 (KAN-196 리뷰 P1-2). 로드 요청은 나가는 순간의 세대 토큰을 들고 가고,
 * 완료 콜백은 그 토큰이 아직 현재 세대일 때만 결과를 받아들인다.
 *
 * ## 왜 필요한가
 *
 ...
 *
 * 순수 클래스로 뺀 이유는 두 게이트(전면·보상형)가 같은 규칙을 쓰고 JVM 테스트가 못박아야 해서다.
 * 스레드 안전하지 않다 — 게이트와 같이 메인 스레드 전용이다.
 */
class AdLoadGeneration {
    private var current = 0

    /** 로드를 시작한다. 돌려준 토큰을 콜백까지 들고 가서 [isCurrent]로 판정한다. */
    fun begin(): Int = current

    /** 지금까지 나간 로드를 전부 옛 세대로 만든다. 그 콜백은 [isCurrent]가 false다. */
    fun invalidate() {
        current += 1
    }

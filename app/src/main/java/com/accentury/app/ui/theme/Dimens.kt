...
object Dimens {
    ...
    /**
     * 막대 두께 (정본 §4). 결과 점수 막대·입력 레벨 바·남은 시간 게이지가 같이 쓴다.
     *
     * 12에서 8로 내렸다 (KAN-161 4단계). 아트보드의 막대가 전부 8이고 진행 도트도 8이라,
     * 화면에 두 굵기가 섞이면 같은 "막대"인데 무게가 달라 보인다. 굵기로 위계를 만들지 않는
     * 팔레트라(정본 §7) 막대는 전부 같은 두께다.
     */
    ...
    /**
     * 화면 위쪽 여백 (정본 §4). 좌우 24·아래 32와 달리 이 값만 8의 배수 밖에 서는 이유는
     * 배치이기 때문이다 — 첫 요소를 이만큼 내려야 그림·제목이 세로 가운데에 앉는다.
     * 웹 `--screen-padding-top`과 같은 값이다 (KAN-161 3단계 웹, 4단계 네이티브).
     */
    val screenPaddingTop = 64.dp

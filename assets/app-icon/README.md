...
## 현재 상태 — 확정 D3 (2026-09-23, 팀장 결정)

`candidates/d3`(한 문장의 피치 곡선 한 획, 잉크 종이 조각 + 크림 스티커 림 + 오프셋 그림자)를
`source.png`·`prompt.md`로 승격했다. 팀장이 2026-09-23에 D3로 확정했고 C2는 탈락했다. 도상을 또
바꾸려면 `source.png`와 `prompt.md`를 그 후보 파일로 갈아끼우고 `build.py`를 다시 돌린다. 스크립트가
`source.png` 하나만 읽으므로 그 밖에 손댈 것은 없다.

| 후보 | 도상 | 원본 | 비고 |
| --- | --- | --- | --- |
| `d3` | 한 문장의 피치 곡선(F0 contour) 한 획, 잉크 종이 조각 + 크림 스티커 림 + 오프셋 그림자 | `candidates/d3/source.png` | **채택(확정)**. 원본 배경 `#F6EDDB`, 도상 안 종이색 `#F7EEDC` — 파생본에서 `#F3ECD9`로 정규화한다. `d3/cream.png`가 그 미리보기 |
| `c2` | "ㅅㅌㄹ" 세 글자 스탬프, 같은 종이 구성 | `candidates/c2/source.png` | 탈락(이력). 원본은 흰 배경(생성 편차). `c2/cream.png`가 정규화 미리보기 |
...
비교 시트와 원본 두 장은 노션 「FE 개발 중 진행 상황 정리」 §1-12에 올려 두고 거기서 결정했다.
...
```
source.png            채택 도상 원본, 생성 도구가 뽑은 그대로 (1024×1024, 배경 #F6EDDB)
...
1. **배경 키잉** — 테두리 평균색을 배경으로 잡고, 그 색과 가까운(채널 차 ≤14) 픽셀 중 **바깥에서 이어진
   영역만** 투명으로 만든다. 도상 안쪽의 같은 색 면은 남는다. 스티커 림은 배경보다 채널 최대 16 어두워서
   벽이 되고(C2는 흰 배경이라 29였다 — D3에서 여유가 줄었으니 허용 오차 `KEY_TOL`을 올릴 때 주의한다),
   그 벽의 작은 틈으로 새는 것은 침식 3px로 끊는다. 경계 픽셀은 배경 거리로 알파를 펴고 섞인
   색을 되돌린다. 여기까지가 `assets/characters/build.py`와 같은 코드다
2. **종이색 정규화** — 도상 안 밝은 면의 중앙값(D3 원본은 `#F7EEDC`)을 기준색으로 잡고 그 색과 가까운
   픽셀만 정확히 `#F3ECD9`로 옮긴다. 기준색은 원본마다 다시 재므로 상수가 아니다. 캐릭터 쪽은 배경이 곧
   종이색이라 휘도 램프로 됐지만, 여기는 종이가 도상 안에만 있어서 기준이 다르다. 휘도 램프를 쓰면 오프셋 그림자(휘도 ~206)까지 밀려 이미 토큰 값인
   `#CFC5AA`에서 벗어난다. **원본은 생성 결과 그대로 두고 손대지 않는다** — 색 보정은 파생본이 책임진다
3. **알파 bbox 트림** → 도상 896×460 (D3 기준. 원본을 갈면 값이 바뀐다)

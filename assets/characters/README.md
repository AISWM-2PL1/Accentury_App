...
2. **종이색 정규화** — 생성 모델은 hex를 정확히 못 맞춘다(원본 5장의 배경이 `#F3ECD9` 대비 +1~+10씩
   제각각). **원본은 생성 결과 그대로 보관하고 손대지 않는다** — "5장 같은 종이색"(KAN-162 Req 2)은
   이 단계가 파생본에서 보장한다: 밝은 픽셀을 토큰 값으로 델타 이동하고 잉크는 그대로 둔다. 결과 WebP의
   캐릭터 종이와 공유 카드 배경은 정확히 `#F3ECD9`다(검증표가 카드 모서리 색을 잰다). 다크 모드에서 캐릭터는 크림 종이 고정 +
...
- **참고 이미지**: **외부·타인 참고 이미지 없음**(KAN-162 저작권 요건 — Dribbble 등 타인 시안을 넣지
  않는다). `-i`로 첨부한 것은 우리가 앞서 뽑은 자체 초안(노션 「결과화면 티어 이미지」 2026-08-27
  버전, 역시 생성 도구 산출물)뿐이다 — 구도·소품을 이어받으려는 자기 참조이지 외부 참고가 아니다.
- **실행**:
  ```

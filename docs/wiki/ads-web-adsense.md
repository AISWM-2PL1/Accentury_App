...
시트는 선택 없이 닫히지 않으므로(§3) 인트로를 거쳐 온 방문은 대개 고른 값을 들고 대기 화면에
선다. 그래도 `unknown`으로 서는 경로가 둘이다 — 인트로를 거치지 않고 대기 화면 주소로 바로
들어온 경우, 그리고 **저장이 막힌 브라우저**다. 뒤쪽은 메모리 사본이 화면 이동 때 사라지기
때문인데(§3.2, PR #109 리뷰 (2026-09-13)에서 정정), 두 경로 모두 이 플래그가 「묻지 않았으면
요청하지 않는다」로 받는다.
...
| 브라우저 웹 — **광고 ID가 든 빌드** | 같은 시트 | **브라우저 저장소** (`localStorage`) | 완료 (2단계, 2026-09-13) |
| 브라우저 웹 — ID 없는 빌드 | **묻지 않는다** (시트도 링크도 없다) | 없음 | 팀 결정 2026-09-13 (PR #109 리뷰) |

**동의는 광고가 있을 때 묻는다.** `VITE_ADSENSE_*`가 빈 빌드는 태그도 슬롯도 서지 않으므로
(§6) 첫 방문자에게 「AdSense 광고가 나와요」를 필수 모달로 띄우면 나가지도 않는 광고를
고지하고 선택까지 강요하는 셈이다. staging과 슬롯 등록 전 prod가 그 상태이고, 테스터는 첫
화면부터 시트에 막힌다. ID가 들어간 배포부터 첫 방문에 묻는다.

방침 10항의 「브라우저 웹에서는 첫 화면의 안내 시트로 동의 여부를 여쭙고」는 그대로 참이다 —
그 문장은 **광고가 나가는 웹**의 서술이고, 광고가 없는 빌드는 그 절이 말하는 대상이 아니다
([`privacy-policy.md`](privacy-policy.md) 10항).
...
| 객체도 `?bridge=`도 없고 (`isStandaloneWeb`) **`adSenseIdsFromEnv() !== null`** | `'web'` | `localStorage` | `adsense` | `readWebAdConsent()` |
| 브라우저 실행인데 광고 ID가 빈 빌드 | `'none'` | 없음 | (`admob`, 의미 없음) | `null` |
...
마지막 관문이 광고 ID다 (팀 결정 2026-09-13, PR #109 리뷰). 판정 함수가 `adsense.ts`의
`adSenseIdsFromEnv`를 그대로 부른다 — 시트를 띄울지와 태그를 심을지가 같은 값 하나를 봐야
「모달은 떴는데 광고는 없는」 빌드가 생기지 않는다.

`consent === null`의 뜻이 2단계에 좁아졌다가 이 리뷰로 한 겹 넓어졌다. 부재는 `'none'` 두
갈래다 — 구버전 앱, 그리고 광고가 나가지 않는 브라우저 빌드다.
...
| 메모리 사본 | 쓰기는 항상 사본에 먼저, 읽기는 사본이 있으면 사본 | 저장소가 없는 환경(사생활 모드·쿼터 초과)에서도 그 화면에서는 선택이 지켜져야 시트가 닫히고 링크가 생긴다. 다음 방문에 다시 묻는 것은 「브라우저 저장소에 둡니다」가 약속한 범위 그대로다 |
| 사본이 사는 범위 | **다음 화면 이동(문서 리로드) 전까지** | 모듈 변수이고 이 앱의 화면 전환이 곧 문서 리로드다 (`web/src/App.tsx`의 `assignHref` — `window.location.href` 대입). 정정 근거는 PR #109 리뷰 (2026-09-13) |
| `writeWebAdConsent`의 반환 | **항상 `true`** | 앱 쪽 `writeAdConsent`의 true가 「네이티브에 닿았다」인 것과 대칭이 아니다 — 이쪽은 「메모리에는 반드시 닿는다」다 |

그래서 **저장이 막힌 브라우저에는 광고가 나가지 않는다.** 인트로에서 「허용」을 골라도 대기
화면은 리로드된 문서라 사본이 없고, 슬롯이 읽는 값은 `unknown`이라 `pauseAdRequests=1`로 요청
자체가 서지 않는다 (§4). 안전한 방향의 실패라 방침 문제는 없다 — 묻지 않은 것처럼 다루는
것뿐이고, 시트는 방문마다 다시 뜬다.
...
| `applyAdConsentToAdSense(consent)` | 이미 선 큐의 플래그만 갱신 | 큐 없음 · 쓰기가 던짐 (리뷰 P1-3) |
| `pushAdSlot()` | `(adsbygoogle ||= []).push({})` | push가 던지면 삼키고 false |

`applyAdConsentToAdSense`만 이 규칙에서 새어 있었다 (리뷰 P1-3, 2026-09-13). 큐는 남이
만들었을 수도 있는 배열이라 — 차단 확장이 동결해 두거나 대역으로 갈아끼운 경우가 있다 —
프로퍼티 대입 한 줄이 TypeError를 낼 수 있고, 부르는 자리가 시트의 `choose`라 거기서 예외가
오르면 사용자가 선택을 바꾼 순간 인트로가 통째로 날아간다. 이제 갱신했으면 `true`,
막혔으면 `false`다.
...
슬롯은 렌더 결과라 이펙트보다 먼저 DOM에 있고, `push({})`가 마지막이다. 「스크립트보다 먼저」를
단언으로 붙든 자리가 `adsense.test.ts`의 「스크립트를 붙이기 전에 플래그가 이미 서 있다」다 —
`appendChild` 호출 순간의 플래그 값을 본다 (리뷰 P1-1). 완료 뒤 상태만 보는 단언은 순서를
뒤집어도 초록이다.

**`push({})`는 `<ins>` 하나당 한 번이다** (리뷰 P1-2, 2026-09-13). 3단계는 중복 push가 던지는
것을 `pushAdSlot`이 삼키는 데 기댔는데, 그 예외는 태그가 **로드된 뒤에만** 나온다. StrictMode의
두 이펙트는 같은 틱에 연달아 돌아 그때 큐는 아직 맨 배열이고, 두 번째 push도 조용히 성공해
슬롯 하나에 요청이 둘 쌓인다. 그래서 `AdSlot`이 `<ins>`에 `data-accentury-pushed="1"`을 남기고
있으면 건너뛴다 — ref가 아니라 DOM 속성인 것은 건너뛴 근거가 개발자 도구에서 보이게 하려는
것이다.
...
**ID 없는 빌드에는 광고에 딸린 것이 하나도 없다** (팀 결정 2026-09-13, PR #109 리뷰). 태그와
슬롯만이 아니라 **동의 시트와 「맞춤형 광고 설정」 링크까지 없다** — `resolveAdConsentSource`가
같은 `adSenseIdsFromEnv`를 보고 `'none'`으로 떨어뜨린다 (§3.1). 로컬·CI·staging·슬롯 등록 전
prod가 전부 이 상태이고, 거기서 첫 화면은 KAN-197 이전과 똑같다.

| ID | 시트·설정 링크 | 태그 | 슬롯 |
|---|---|---|---|
| 둘 다 있다 | 첫 방문에 뜬다 | 대기 화면에서 선다 | `<ins>`가 선다 |
| 하나라도 없다 | **없다** | 없다 | 없다 (상자째 없다) |
...
(`${ADSENSE_CLIENT_ID#ca-}`). 그리고 값이 `^ca-pub-[0-9]+$`에 맞지 않으면 **스텝이 실패한다** —
틀린 게시자 ID가 적힌 파일이 조용히 올라가면 소유권 확인이 안 되는데, 증상이 「심사가 아직 안
끝났다」와 구분되지 않아 몇 주를 잃는다.

검사를 접두어에서 정규식으로 좁혔다 (리뷰 P1-4, 2026-09-13). 접두어만 보면 `ca-pub-` 자체나
`ca-pub-12ab`가 통과해 「`pub-`」 한 줄짜리 쓸모없는 파일이 올라간다. **자릿수는 고정하지
않는다** — 리뷰 제안은 `[0-9]{16}`이었으나 16은 Google 문서가 보장한 계약이 아니라 지금 발급되는
값의 관찰일 뿐이라, 자릿수가 늘면 멀쩡한 게시자 ID로 배포가 죽는다. 막아야 할 것은 「숫자가 아닌
값」이지 「긴 값」이 아니다.

셸 조각을 다섯 경우로 돌려 본 결과다 (2026-09-13 재실측).
...
| `ca-pub-` | **exit 1** · 파일 없음 (숫자가 하나도 없다) |
| `ca-pub-12ab` | **exit 1** · 파일 없음 (숫자 아닌 문자가 섞였다) |
...
| 나간 광고 요청 URL에 전부 `npa=1` | 거부한 방문이 비맞춤으로 나간다 (AC 3, 리뷰 P1-1). 아래 참고 |

**요청 URL 단언이 하나 늘었다** (리뷰 P1-1, 2026-09-13). 단위 테스트가 보는 것은 큐 프로퍼티
`requestNonPersonalizedAds=1`까지이고, 그 값이 실제 요청 URL의 `npa=1`이 되는지는 진짜 브라우저와
진짜 스크립트가 있어야 나온다 — 위 §7의 사람 절차를 자동화한 자리다. 스펙이
`page.on('request')`로 `googlesyndication.com`·`doubleclick.net`의 **슬롯 요청**(`/pagead/ads?`)만
모아 전부 `npa=1`을 담았는지 본다. 태그 스크립트 자체(`adsbygoogle.js?client=`)와 같은 호스트의
설정·측정 엔드포인트는 광고를 달라는 요청이 아니라 npa가 붙을 자리가 없어 뺀다.

**더미 ID로도 요청이 나간다**(2026-09-13 실측 2건).

```
https://googleads.g.doubleclick.net/pagead/ads?npa=1&client=ca-pub-0000000000000000&output=html&…
https://googleads.g.doubleclick.net/pagead/ads?npa=1&client=ca-pub-0000000000000000&…&slotname=0000000000&…
```

승인 전이라 **광고가 안 채워지는 것**과 요청이 **안 나가는 것**은 다른 사건이고(§1), 채워지지
않는 요청에도 npa는 실린다. 그래도 0건은 통과로 둔다 — 요청이 나가는지는 네트워크·차단
확장·Google 쪽 응답에 달려 우리가 통제하지 못하고, 「광고 요청이 없다」 자체는 이 단언이 잡을
실패가 아니다(그 판정은 첫 줄의 슬롯 가시성이 맡는다). 몇 건이 관찰됐는지는 스펙이 로그로
...
### 10.1 내릴 때 — 변수를 지우는 것으로 부족하다 (리뷰 P2-2)

위 표는 켜는 절차다. 끄는 쪽은 대칭이 아니라 따로 적는다.

| 할 일 | 왜 |
|---|---|
| `gh variable delete ADSENSE_CLIENT_ID -e prod` (슬롯 ID도 함께) | 다음 배포부터 번들에 태그가 빠진다 (`adSenseIdsFromEnv`가 null) |
| `aws s3 rm s3://<WEB_BUCKET>/ads.txt` | **여기가 빠지기 쉬운 칸이다.** 변수를 지우면 워크플로는 파일을 새로 만들지 않을 뿐 S3에 있던 객체는 그대로 둔다 — sync에 `--delete`가 없고 배포 역할에 DeleteObject 권한도 없다 (§6.2). 광고를 내렸는데 「이 게시자가 이 도메인을 판다」는 선언만 남는다 |
| CloudFront `/ads.txt` 무효화 | 이름에 해시가 없어 지운 뒤에도 엣지 캐시가 옛 내용을 계속 준다 |

**실행 권한은 infra 담당에게 있다.** 배포 역할에 DeleteObject가 없는 것이 실수가 아니라
설계라서다 — 배포가 버킷의 객체를 지울 수 있으면 잘못된 한 판이 사이트를 지운다. 두 번째·세 번째
칸은 사람이 관리자 자격으로 한 번 하는 일이다.

...
창이 대역 안에 들어와야 창 밖 값이 전부 clamp로 간다. ±7 semitone은 ×0.56~×1.78이라, 60~800Hz면 **중심 107~449Hz 화자는 창 전체가 대역 안**이다 — 낮은 남성음(vocal fry 하한 60Hz대)부터 여성 감탄·고성(800Hz)까지 들어온다. 대역이 창보다 좁은 화자는 이제 없다고 봐도 된다. 창 밖 값을 어디에 그릴지는 추정기가 아니라 표시 쪽이 정한다는 역할 분담이 이것으로 성립한다.

### 옥타브 위 오류 위험과 실측

상한을 올리면 τmin이 40→20이 되어 **반주기 골**(옥타브 위 오류)을 먼저 만날 위험이 생긴다. CMNDF의 누적 정규화가 작은 τ의 골을 임계값 위로 띄워 주는 것이 YIN의 설계 의도이고, 실측으로 확인했다.

| 검증 | 결과 |
|---|---|
| 2·3배음이 기음보다 큰 합성 신호, 60~800Hz 10Hz 간격 (웹 `yin.test.ts`) | 옥타브 오류 0건 |
| 450·520·600Hz 배음 신호, 62·70Hz 유성, 790/820Hz 경계 (Android·iOS 테스트) | 통과 |
| 에뮬레이터 합성 목소리 150→600→65→150Hz (배음 1:0.5:0.3, 비브라토 ±0.3st) 로그 f0 | 600 구간 601~606Hz, 65 구간 64.4~65.0Hz — null·뒤집힘 없음 |

...
조사·결정 기록 (2026-08-13). 실시간 피치 곡선용 온디바이스 F0 추출기를 확정한다.
...
1. **라이선스가 결정적 제약**. TarsosDSP·aubio는 GPLv3, Essentia는 AGPLv3. GPL 계열은 APK/IPA 배포 순간 카피레프트가 걸려 앱 전체 소스공개 의무 발생 → 상용 클로즈드소스 앱에서 즉시 탈락. 이 한 축이 "기성 라이브러리 붙이기" 경로를 대부분 제거한다.

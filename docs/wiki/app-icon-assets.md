...
- **파비콘·OG 이미지** — KAN-179가 같은 도상을 재사용한다(`web-link-preview.md`, `assets/web/build.py`). 도상이 바뀌면 그쪽도 같이 다시 뽑고 파일명의 `VERSION`을 올려야 한다 — 웹 자산은 1년 immutable 캐시라 이름을 안 바꾸면 옛 그림이 계속 나간다.

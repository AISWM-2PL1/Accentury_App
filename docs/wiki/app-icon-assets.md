...
**확정 D3** (2026-09-23, 팀장 결정). 한 문장의 피치 곡선을 한 획으로 그어 잉크 종이 조각에 얹고
크림 스티커 림과 오프셋 그림자를 두른 그림이다. `assets/app-icon/candidates/d3`의 `source.png`와
`prompt.md`를 `assets/app-icon/` 위로 승격했다.

팀장이 2026-09-23에 D3로 확정했고, 그때까지 임시 확정이던 C2("ㅅㅌㄹ" 세 글자 스탬프)는 탈락했다.
도상을 또 바꾸려면 `source.png`와 `prompt.md`를 그 후보 파일로 갈아끼우고 `build.py`를 다시 돌린다 —
스크립트가 `source.png` 하나만 읽으므로 그 밖에 손댈 것이 없다. 탈락한 C2와 두 후보를 런처
크기(400/144/96/72/48px)로 나란히 놓은 비교 시트는 `candidates/`에 이력으로 남아 있다.

스토어 자산도 이 확정으로 같이 재생성됐다. Play 아이콘 512와 피처 그래픽은 `store/`의 생성물이고,
App Store 쪽은 Xcode가 에셋 카탈로그의 `AppIcon`에서 자동으로 뽑는다 — `store/app-store-icon-1024.png`는
업로드 편의 사본일 뿐이다.
...
| 층 | 위치 | 성격 |
|---|---|---|
| 원본 | `assets/app-icon/source.png` + `prompt.md` | 생성 도구가 뽑은 그대로(1024×1024, 배경 **`#F6EDDB`**). 손으로 고치지 않는다 |
...
이하이면서 **바깥에서 이어진** 영역만 투명으로 만든다(flood-fill). 스티커 림이 배경보다 어두워
벽이 되고, 그 벽의 틈으로 새는 것은 침식 3px로 끊는다. 그 여유는 원본마다 다르다 — 흰 배경이던
C2는 채널 최대 29였고 D3는 16이다. 허용 오차 `KEY_TOL`을 올릴 일이 생기면 새 원본에서 림과 배경의
채널 차를 먼저 재고 올린다. 경계 픽셀은 배경 거리로 알파를
펴고 섞인 색을 되돌린다.

**종이색 정규화만 캐릭터 쪽과 다르다.** 캐릭터는 배경이 곧 종이색이라 "휘도가 높을수록 델타 적용"
휘도 램프로 됐다. 여기는 종이가 도상 안에만 있어서, 기준색을 배경이 아니라 **도상 안 밝은 면(휘도
230 초과)의 중앙값**(D3 원본은 `#F7EEDC`)에서 잡고 그 색과의 거리로 가중치를 준다. 기준색은 실행마다
원본에서 다시 재므로 상수가 아니다.
...
- **C2 임시 확정** (2026-09-03) — 팀 결정 전까지 쓰던 값이다. 아래 D3 확정이 대체했다.
- **도상 D3 확정** (2026-09-23, 팀장) — C2(가로 "ㅅㅌㄹ" 세 글자)와 D3(피치 곡선 한 획) 중 D3. `source.png`를 `candidates/d3/source.png`로 갈고 `build.py`를 다시 돌려 Android·iOS·스토어 파생본을 전부 재생성했다. 웹 파비콘·OG도 같은 원본이라 `assets/web/build.py`의 `VERSION`을 `v2`로 올려 같이 다시 뽑고 `web/index.html`의 참조를 따라 고쳤다 — 이름을 안 바꾸면 1년 immutable 캐시가 옛 그림을 계속 준다.
...
- **인트로에서 중복 제목을 걷고 히어로에 h1을 넘겼다** (2026-09-03) — 히어로 `사투리 좀 치나?` 바로 아래에 제목 `사투리 억양 테스트`가 서서 같은 말을 두 번 했고, 사용자가 두 줄에서 새로 얻는 것이 없었다. 제목을 지우고 히어로를 이 화면의 h1으로 세운다(`TextHero`의 `heading`) — 화면 이름을 말하는 것이 히어로뿐이 되므로 `aria-hidden`을 그대로 두면 인트로가 접근 가능한 이름과 유일한 h1을 잃는다. 분석 대기 화면은 상태를 실어 나르는 h1이 아래에 따로 있어 기본값(장식)을 그대로 쓴다. 인트로만 192px 슬롯에서 히어로를 꺼내 부제와 8px로 묶었고, `raw/{android,ios}/01-intro.png`를 다시 찍어 프레임을 재생성했다.
...
- **실기기 캡처** — 현재 스크린샷은 에뮬레이터(Android)·시뮬레이터(iOS) 캡처다. AC1은 이것으로 충족하지만, 실기기 캡처로 갈아 끼우면 더 낫다. 캡처만 `raw/`에 덮고 다시 돌리면 된다. Android `01-intro`·`02-recording`·`03-vocab`는 2026-09-22에 다시 찍었다(광고 동의 시트·IITP 표기·곡선 대역 반영). `raw/android/04-result`와 `raw/ios/*`는 2026-09-03 캡처 그대로다.
- **Android `04-result` 재촬영** — 아직 못 했다. 가짜 마이크(`-PfakeMic`)로 prod에 붙으면 음성 5문항이 전부 "분석 실패"로 떨어져 결과 화면까지 못 간다(단어 5문항만 통과해 "분석 중 5 / 10"에서 멈춘다). prod AI는 실제 발화를 기대하는데 가짜 마이크가 흘리는 WAV는 문항 문장과 다른 소리라서다 — 2.5초 원본과 7.5초(3배 이어붙임) 양쪽 모두 같은 결과였다. 결과 화면을 찍으려면 AI 스텁을 띄운 로컬 스택에 붙이거나(`ticket-emu` 루틴), 실기기에서 사람이 직접 읽어 완주해야 한다.
- **스토어 등록** — Play(KAN-174, `play-store-listing.md` §3)·App Store(KAN-175, [`app-store-listing.md`](app-store-listing.md) §4)에 이 파일들을 올리는 것은 별도 티켓이다. App Store는 1024 아이콘을 따로 받지 않고 빌드의 에셋 카탈로그에서 꺼내 쓴다.
- **파비콘·OG 이미지** — KAN-179가 같은 도상을 재사용한다(`web-link-preview.md`, `assets/web/build.py`, 현재 `VERSION = "v2"`). 도상이 또 바뀌면 그쪽도 같이 다시 뽑고 파일명의 `VERSION`을 올려야 한다 — 웹 자산은 1년 immutable 캐시라 이름을 안 바꾸면 옛 그림이 계속 나간다.

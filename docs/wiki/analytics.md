...
| `retest_started` | `from` | 웹 | [다시 테스트하기]. `from`은 누른 자리 — `result`(결과를 다 보고 한 번 더) / `waiting`(분석이 막다른 상태라 되돌아 나옴, KAN-191) / `item`(문항 제출이 세션 만료·타인 세션으로 거절돼 나옴, KAN-237) |
...
> GA4·Crashlytics는 여전히 광고 ID를 붙이지 않는다(`ads-admob.md` §6·§7.1). 바뀐 것은 AdMob 몫이
> 따로 신고 항목에 더해진다는 것이다.
...
**계측만 보면 «추적(Tracking)»에 해당하지 않는다** — 이 문단은 GA4·Crashlytics에 한한 판단이다. 계측은
IDFA를 수집하지 않고 광고 신호를 껐으므로 계측 때문에 App Tracking Transparency 프롬프트가 필요하지는
않다 — 위 §3의 `otool`·`nm` 확인이 그 근거다(단, 그 확인은 `FirebaseAnalyticsCore` 한정이고 AdMob SDK가
AdSupport·ATT를 링크하므로 바이너리 전체에는 더는 성립하지 않는다 — `ads-admob.md` §7.1). `FirebaseAnalytics`
product로 되돌리면 계측 쪽 판단도 뒤집히므로, 그 변경은 이 절과 함께 다시 읽어야 한다. 앱 전체의 답은
위 머리글대로 `privacy-policy.md` §2가 정본이다.
...
- **공유 전송 완료 수** - 카카오 공유 웹훅(서버 콜백)으로 BE가 센다 (KAN-164, `POST /v0/share/kakao/webhook`, 조회는 `GET /admin/v0/analytics`의 `shares`). 앱이 `serverCallbackArgs`에 `campaign=kko_share`를 실어야 콜백이 온다. Firebase 이벤트가 아니라 서버 카운터라 이 문서의 퍼널과는 별도 표다.

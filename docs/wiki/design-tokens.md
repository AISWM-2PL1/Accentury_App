...
(`니 사투리 몇 등급?`)와 분석 대기 화면(`분석 중입니다`)이 쓴다 — KAN-178에서 두 화면의 종이
일러스트를 걷어낸 자리다. 스토어 캡처 크기로 줄면 오려 낸 선이 뭉개져 무엇을 그린 것인지
읽히지 않았고, 읽히더라도 앱이 무엇을 하는 곳인지는 말하지 않았다.

모양은 대사 카드(`.prompt-card`)에서 그대로 빌린다 — 1.5px 잉크 테두리, 크림 면, `radius-xl`,
`shadow-prompt`. 한 화면에 종이가 두 종류로 보이지 않으려면 두께와 그늘이 같아야 한다. 글자는
`font-display` `text-display`(40) 400이다: 바로 아래 제목이 30·26이라 이보다 작으면 히어로가
아니라 제목이 둘인 화면이 된다.

기본값이 `aria-hidden`인 이유는 옆에 있는 h1이 이미 화면 이름을 말하기 때문이다. 장식에 라벨을
붙이면 스크린 리더가 같은 화면을 두 번 소개한다.

바깥 슬롯(`.illustration--intro` 192 · `--waiting` 176)의 높이는 건드리지 않았다. 글자 조각이

...
글자는 `font-display` `text-display`(40) 400이다: 바로 아래 제목이 30·26이라 이보다 작으면
히어로가 아니라 제목이 둘인 화면이 된다.

기본값이 `aria-hidden`인 이유는 옆에 있는 h1이 이미 화면 이름을 말하기 때문이다. 장식에 라벨을
붙이면 스크린 리더가 같은 화면을 두 번 소개한다.

바깥 슬롯(`.illustration--intro` 192 · `--waiting` 176)의 높이는 건드리지 않았다. 글자 한 줄이
그림보다 낮아도 아래 제목·카드는 있던 자리에 그대로 남는다.

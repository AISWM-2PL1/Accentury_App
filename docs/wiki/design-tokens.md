...
**Papercut** — 크림 종이 위에 잉크로 그린 그림이고, 두 번째 잉크로 버밀리언 한 색을 더 쓴다.
...
| VERMILION | `#b3341f` | **포인트 컬러.** 화면당 한 자리 — 지금 눌러야 할 것과 지금 여기를 짚는다 (§7) |
...
| `point` | `#b3341f` | **포인트 컬러 면.** 주버튼·제목 밑줄·강조 사선. 화면당 한 자리 (§7) |
...
| `point-foreground` / `point` (라이트) | 5.20 |  |
| `point` / `background` (라이트) | 5.20 |  |
...
| `point-foreground` / `point` (다크) | 5.20 |  |
| `point` / `background` (다크) | 5.20 |  |
...
| 후보 | 크림 위 | 잉크 옆 | |
|---|---|---|---|
| **버밀리언 `#b3341f`** | **5.20** | 2.83 | 채택 |
| 테라코타 `#a8503a` | 4.60 | 3.20 | 잉크와 가장 잘 갈리지만 종이에 묻힌다 |
...
버밀리언은 크림 위 5.20:1로 본문 기준(4.5:1)을 넘으므로 글자에도 쓸 수 있고, 위에 크림 글자를
얹어도 같은 5.20:1이라 주버튼 면이 된다 (§6).

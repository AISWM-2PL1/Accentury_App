...
| `RELEASE_KEY_ALIAS` | `accentury` | 박유현 |
...
- **Play App Signing 등록.** 아직 안 했다. 등록하면 이 키스토어는 **업로드 키**가 되고 배포용 서명
  키는 구글이 들고 있게 된다 — 업로드 키는 잃어도 재발급받을 수 있다는 것이 이득이다.
  **함정**: 등록 이후 `assetlinks.json`(KAN-32)과 카카오 콘솔에 넣을 지문은 이 키스토어의 지문이
  아니라 **구글이 들고 있는 앱 서명 인증서의 지문**이다. 등록하는 사람이 두 곳을 함께 갈아야 한다.

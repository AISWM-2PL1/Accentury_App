...
/// 다른 자리에 있기 때문**이다. 앱은 ``RecordingEngine/maxDurationMs``로 끊고, 웹은 문항
/// 정의의 `maxDurationMs`(서버가 준 값)로 버퍼를 잡는다. 창은 그 둘 중 **이 화면에서 실제로
/// 적용되는 쪽**을 받아야 창의 오른쪽 끝이 닿을 수 있는 자리가 된다.
///
/// 그래서 앱이 브리지로 받은 ``VoiceItemStart/maxDurationMs``를 여기 넘기지 않는 것은
/// 의도다 - 넘기면 서버가 15초를 주는 날 창은 15초인데 녹음은 10초에 끊겨 이 함수가 고친
/// 바로 그 문제로 되돌아간다. 녹음 엔진이 서버 값을 읽지 않는 것 자체는 KAN-195 범위 밖이고,
...
public func reviewWindow(
     ...
     * 그릴 점이 애초에 없으므로(``userCurveDisplayPoints(_:windowMs:centerHz:)``가 무성
     * 프레임에서 점을 만들지 않는다) 창을 어떻게 잡든 결과가 같다.
     */
    guard let first = firstVoicedMs, let last = lastVoicedMs else {

...
/// OS 공유 시트에 실을 본문 (KAN-214). 안드로이드 `ResultSharer.kt`·웹 `shareResult.ts`의
/// `systemShareText`와 같은 이름·같은 모양이다 — 셋 중 하나만 바뀌면 눈에 띄게 두려는 것이고,
/// 같은 공유가 플랫폼마다 다른 모양으로 도착하는 것이 KAN-30이 피하려는 일이다.
///
/// 문구와 링크를 줄바꿈으로 이어 **한 문자열**로 만든다. 예전에는 시트에 문구와 `URL` 항목을
/// 따로 실었는데, 카톡 공유 확장처럼 항목마다 메시지를 만드는 수신 앱에서는 그게 링크 버블
/// 하나와 문구 버블 하나, 두 번 보낸 꼴이 된다 — 받는 쪽에서는 한 번의 공유가 두 개로 쪼개져
/// 도착한다. 안드로이드는 처음부터 `"문구\n링크"` 한 덩어리였고 웹도 같은 티켓에서 맞췄다.
///
/// 대신 `URL` 항목을 따로 읽어 미리보기 카드를 그리는 앱에서는 링크가 본문 텍스트로만 들어간다.
/// 메시지가 둘로 갈리는 것보다 이쪽이 낫다고 보고 감수한다.
///
/// 점수는 싣지 않는다 (KAN-30 요구) — 재료가 서버가 준 문구와 캠페인 URL뿐이라는 것이 함수
/// 하나로 보이는 편이 낫다.
func systemShareText(_ payload: SharePayload) -> String {
    "\(payload.text)\n\(payload.webTestUrl)"
}

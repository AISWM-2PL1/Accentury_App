...
///
/// **복구 경로는 [재시도] 하나다** (KAN-147). [테스트 종료] 버튼은 없다. 재시도 불가 실패에도
/// 버튼을 주지 않는다: 같은 바이트를 다시 보내도 결과가 같아 버튼이 거짓말이 되고, 이 바는
/// 정상 진행 중에 떠 있는 화면이라 이탈 버튼을 달면 KAN-147이 지운 것을 되살린다. 그 대신
/// 서버가 준 문구가 행에 그대로 남고, 출구는 분석 대기 화면에 있다 — [다시 녹음], 세션 단위
/// 실패면 [다시 테스트하기] (KAN-191, ux-ui.md §4-D·§4-F).
///
